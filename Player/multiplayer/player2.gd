...
func _ready():
	...
	#regenerete_digit_prefab = preload("res://effects/life_digit/LifeRegenerete.tscn")
	
func _process(delta: float) -> void:
	#Injetando a posição do player no Game_Manger
	GameManager.player_position2 = position
	...
	if Input.is_action_just_pressed('attack_2'):
		if Input.is_action_pressed("down_2"):
			attack("attack_down_")
		elif Input.is_action_pressed("up_2"):
			attack("attack_up_")
		else:
			attack("attack_side_")
...
func read_input() -> void:
	...
	input_vector = Input.get_vector('left_2','right_2', 'up_2', 'down_2', 0.15)
		...
		if area.get_parent().is_in_group("enemies"):
			if area.name == 'AtackArea':
				...
				if not enemy.is_attacking  and not die_player:
					...
					#Esta comentando pois o valor esperado na variavel player do inimigo é player
					#esse codigo tem a class player2(PRECISA SER MUDADO PARA TER UM CODIGO SÓ)
					#enemy.player = self
